...
const categoryMeta: Record<string, { icon: typeof Bell; color: string }> = {
  order: { icon: Bell, color: 'bg-emerald-50 text-emerald-600' },
  chat: { icon: MessageSquare, color: 'bg-blue-50 text-blue-600' },
  ...
  if (notifications && notifications.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-stone-400">
        ...
        <p className="text-xs text-stone-400 mt-1">Notifikasi akan muncul di sini</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 space-y-4">
      {Object.entries(grouped).map(([date, items]) => (
        <div key={date}>
          <p className="text-xs font-semibold text-stone-500 px-1 mb-2">{date}</p>
          <Card className="rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
            <CardContent className="p-0 divide-y divide-stone-100">
              {items.map((notif) => {
                const meta = categoryMeta[notif.category] || { icon: Bell, color: 'bg-stone-50 text-stone-600' };
                const Icon = meta.icon;
                return (
                  <button
                    key={notif.id}
                    onClick={() => handleNotifClick(notif)}
                    className="w-full flex items-start gap-3 p-4 text-left hover:bg-stone-50 transition-colors duration-200"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <h4 className={`text-sm ${notif.is_read ? 'font-medium text-stone-700' : 'font-bold text-stone-900'}`}>
                          {notif.title}
                        </h4>
                        {!notif.is_read && (
                      ...
                      <p className="text-[11px] text-stone-400 mt-1">{formatTime(notif.created_at)}</p>
                    </div>
                  </button>
                );
              })}
...
export default function VendorNotificationsPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-stone-50">
      <div className="bg-gradient-to-b from-emerald-700 to-emerald-600 text-white p-4 pt-8 rounded-b-[24px] shadow-sm shrink-0">
      ...
      <Suspense fallback={<div className="flex items-center justify-center py-20"><Loader2 size={24} className="animate-spin text-stone-400" /></div>}>
        <NotifContent />
      </Suspense>
    </div>
  );
